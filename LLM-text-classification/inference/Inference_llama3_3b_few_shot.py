...
# === 환경 설정 ===
os.environ["CUDA_VISIBLE_DEVICES"] = "2"  # 사용할 GPU 지정
...
def predict_label_with_pipe(title_content, candidate_labels):
    ...
    outputs = pipe(
        messages,
        max_new_tokens=128,  # 줄여서 OOM 방지
    )
    out = outputs[0]["generated_text"][-1]
...
# 결과 저장 경로
save_path = "/home/yena/Classification_module/llama3-3b_few shot_valdata.csv"
...
# 메인 루프
for index, row in tqdm(total_data.iterrows(), total=total_data.shape[0]):
    if row["제목+내용"] in already_done:
        continue  # 이미 처리된 샘플은 건너뜀
    category = row["대분류"]
    subcategory = row["중분류"]
    question_text = row["제목+내용"]

    filtered_df = classification_data[(classification_data["대분류"] == category) & (classification_data["중분류"] == subcategory)]
