...
- Leadtime: 87:37 train/test split (7:3 ratio) + lognormal distribution (data_manager.py 방식)
...
# ============================================================================
# ★★★ Leadtime Split & Fit (LOGNORMAL) ★★★
# ============================================================================
def fit_lognormal_dist(data: np.ndarray, name: str = "data"):
    """
    Lognormal 분포 피팅 (data_manager.py 방식)
    """
    data = np.array(data)
    data = data[data > 0]
    ...
    # Lognormal fitting: shape = sigma, scale = exp(mu)
...
def split_leadtime_and_fit_lognormal(leadtime_data: np.ndarray, train_ratio: float = 0.7):
    """
    data_manager.py 방식: leadtime을 7:3으로 시계열 순서대로 분할 후 lognormal 적합
    Returns:
        dict: {
            'train_params': {'mu': ..., 'sigma': ...},
            'test_params': {'mu': ..., 'sigma': ...},
            'train_count': int,
            'test_count': int
        }
    """
    ...
    # Lognormal 분포 피팅
    train_params = fit_lognormal_dist(lt_train, name="train")
    test_params = fit_lognormal_dist(lt_test, name="test")
...
def load_item_params_with_leadtime_split(
    ...
    """
    params_master.json 로드 + leadtime train/test 분할 (LOGNORMAL)
    Args:
    ...
    if leadtime_raw_data is not None and len(leadtime_raw_data) > 0:
        ...
        lt_split = split_leadtime_and_fit_lognormal(leadtime_raw_data, train_ratio=train_ratio)
        
        if use_train:
            leadtime_params = lt_split['train_params']
            count = lt_split['train_count']
            print(f"[INFO] Using TRAIN leadtime ({count} samples): lognormal(mu={leadtime_params['mu']:.4f}, sigma={leadtime_params['sigma']:.4f})")
        else:
            leadtime_params = lt_split['test_params']
            count = lt_split['test_count']
            print(f"[INFO] Using TEST leadtime ({count} samples): lognormal(mu={leadtime_params['mu']:.4f}, sigma={leadtime_params['sigma']:.4f})")
        ...
        if lt_family == "lognorm":
            ...
            print(f"[WARNING] Converting gamma to lognorm for {item_key}")
...
# ============================================================================
def split_dataframe_item4(df: pd.DataFrame):
    """
    Item4용 342:57:171 (train:valid:test) 순차 분할 - 6:1:3
    """
...
class WeeklyInvEnv(gym.Env):
    ...
    def __init__(
        ...
        self.lt_horizon_weeks = pipeline_horizon  # WeeklyInvEnv
    ...
    def step(self, aidx):
        ...
        reward = (-holding - back_cost - order_cost) / 5000.0
...
# ============================================================================
# GenerativeInvEnv (Training용) - LOGNORMAL
# ============================================================================
class GenerativeInvEnv(gym.Env):
    """★ LOGNORMAL leadtime + nbinom demand 기반 생성적 환경 ★"""
    def __init__(
        self,
        item_params: dict,
        episode_len: int = 342,
...
def main():
    ...
    parser.add_argument("--lt_horizon_weeks", type=int, default=45)
    ...
    parser.add_argument("--reward_scale", type=float, default=5000.0, help="Reward scaling factor (reward = -cost / reward_scale)")
    parser.add_argument("--action_unit", type=int, default=1)
    parser.add_argument("--max_order", type=int, default=300)
    parser.add_argument("--episodes", type=int, default=37800)
    parser.add_argument("--eval_freq_episodes", type=int, default=20)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--output_dir", type=str, default="./outputs")
    parser.add_argument("--lr", type=float, default=5e-5)  # 3e-4 → 5e-5 (매우 안정적)
    parser.add_argument("--n_steps", type=int, default=2048)
    parser.add_argument("--batch_size", type=int, default=256)  # 64 → 256 (대폭 증가)
    parser.add_argument("--n_epochs", type=int, default=4)  # 10 → 4 (overfitting 방지)
    parser.add_argument("--gamma", type=float, default=0.99)
    parser.add_argument("--gae_lambda", type=float, default=0.95)
    parser.add_argument("--clip_range", type=float, default=0.08)  # 0.2 → 0.08 (매우 보수적)
    parser.add_argument("--ent_coef", type=float, default=0.05)  # 0.01 → 0.05 (exploration 증가)
    ...
    # Output 디렉토리
    output_dir = os.path.join(args.output_dir, f"item{item_key}_PPO_Lognormal_TrainLT_All")
    ...
    df_train, df_valid, df_test = split_dataframe_item4(df_weekly)
    ...
    # WandB
    run_name = f"yenaper_item{item_key}_PPO_Lognormal_ep{episode_len}_342-57-171split_TrainLT_All"
    wandb.init(
    ...
    print(f"  - Train: {len(train_traj_df)} weeks (indices 0-{len(train_traj_df)-1})")
    ...
    if os.path.exists(best_model_path + ".zip"):
        ...
        print(f"\n[INFO] ========== 16 Visualizations Complete ==========")
        ...
        print(f"[INFO] - Total: 16 visualizations")
        ...
        # 로깅 업데이트
        wandb.log(vis_log_dict)
        print("[INFO] 20 Visualizations Complete")
        print("[INFO]   - Reward per step: 4 plots (Train/Valid/Test/All)")
        print("[INFO]   - On-hand & Backlog: 4 plots (Train/Valid/Test/All)")
        print("[INFO]   - Order Quantity: 4 plots (Train/Valid/Test/All)")
        print("[INFO]   - Order Distribution: 4 plots (Train/Valid/Test/All)")
        print("[INFO]   - Demand: 4 plots (Train/Valid/Test/All)")  
        print("[INFO]   - Total: 20 visualizations")
        print(f"[INFO] All visualizations saved to {output_dir}")
    ...
    print(f"\n[INFO] Leadtime configuration:")
    if item_params_train['leadtime']['family'] == 'lognorm':
        print(f" - Train: lognormal(mu={item_params_train['leadtime']['mu']:.4f}, sigma={item_params_train['leadtime']['sigma']:.4f})")
    elif item_params_train['leadtime']['family'] == 'gamma':
        print(f" - Train: gamma(k={item_params_train['leadtime']['k']:.4f}, theta={item_params_train['leadtime']['theta']:.4f})")
    else:
        print(f" - Train: {item_params_train['leadtime']}")
    print(f" - Valid: SAME AS TRAIN")
    print(f" - Test: SAME AS TRAIN")
    print(f"[INFO] ============================================")
