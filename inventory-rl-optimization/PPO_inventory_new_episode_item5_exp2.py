...
# ============================================================================
# ★★★ Leadtime Split & Fit (GAMMA) ★★★
# ============================================================================
def fit_gamma_dist(data: np.ndarray, name: str = "data"):
    """
    Gamma 분포 피팅 (data_manager.py 방식)
    """
    data = np.array(data)
    data = data[data > 0]
    ...
    print(f"[INFO] {name.capitalize()} gamma fit: k={k:.4f}, theta={theta:.4f}")
    return {'k': float(k), 'theta': float(theta)}
...
def split_leadtime_and_fit_gamma(leadtime_data: np.ndarray, train_ratio: float = 0.7):
    ...
    # Gamma 분포 피팅
    train_params = fit_gamma_dist(lt_train, name="train")
    test_params = fit_gamma_dist(lt_test, name="test")
...
def load_item_params_with_leadtime_split(
    ...
    """
    params_master.json 로드 + leadtime train/test 분할 (GAMMA)
    Args:
    ...
    if leadtime_raw_data is not None and len(leadtime_raw_data) > 0:
        ...
        if lt_family == "gamma":
            ...
            # Lognormal을 Gamma로 변환 (필요시)
            print(f"[WARNING] Converting lognorm to gamma for {item_key}")
            mu, sigma = lt_params['mu'], lt_params['sigma']
    ...
    if demand_family == "nbinom":
        ...
        # Poisson을 NB로 근사: r을 매우 크게, p = r/(r+lambda)
        r_approx = 1000000.0
        ...
        print(f"[INFO] Converted Poisson(lam={lam:.2f}) to HurdleNB(r={r_approx:.0f}, p={p_approx:.6f})")
    else:
...
# ============================================================================
def split_dataframe_item5(df: pd.DataFrame):
    """
...
# ============================================================================
# WeeklyInvEnv (Test용) - GAMMA leadtime 샘플링
# ============================================================================
class WeeklyInvEnv(gym.Env):
    """CSV 기반 Test 환경 + GAMMA leadtime"""
    def __init__(
        self,
    ...
    def step(self, aidx):
        ...
        reward = (-holding - back_cost - order_cost) / 1000.0
...
# ============================================================================
# GenerativeInvEnv (Training용) - GAMMA
# ============================================================================
class GenerativeInvEnv(gym.Env):
    """★ GAMMA leadtime + Poisson/nbinom demand 기반 생성적 환경 ★"""
    def __init__(
        self,
        item_params: dict,
        episode_len: int = 342,
        ...
        self.lt_family = lt_data.get("family", "gamma")
    ...
    def sample_demand(self) -> float:
        """HurdleNB (pi=0 → nbinom or Poisson)"""
...
def main():
    ...
    parser.add_argument("--lt_horizon_weeks", type=int, default=42)
    ...
    # ★ Item5 + GAMMA leadtime + POISSON demand 파라미터 로드 ★
    ...
    # Demand configuration 출력
    print(f"[INFO] ========== DEMAND CONFIGURATION ==========")
    if item_params_train['demand']['family'] == 'HurdleNB':
        if item_params_train['demand']['r'] > 100000:  # Poisson approximation
            print(f"[INFO] Demand: Poisson-like (HurdleNB with large r={item_params_train['demand']['r']:.0f})")
        else:
            print(f"[INFO] Demand: Negative Binomial (r={item_params_train['demand']['r']:.2f}, p={item_params_train['demand']['p']:.4f})")
    print(f"[INFO] ============================================\n")
    
    # Output 디렉토리
    output_dir = os.path.join(args.output_dir, f"item{item_key}_PPO_Gamma_Poisson_TrainLT_All")
    ...
    df_train, df_valid, df_test = split_dataframe_item5(df_weekly)
    ...
    # WandB
    run_name = f"hooboitem{item_key}_PPO_Gamma_Poisson_ep{episode_len}_342-57-171split_TrainLT_All"
    wandb.init(
