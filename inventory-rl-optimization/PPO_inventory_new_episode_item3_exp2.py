...
Item3 specific:
- Demand: 342:57:171 train/valid/test split (6:1:3) + nbinom distribution
- Leadtime: 87:37 train/test split (7:3 ratio) + gamma distribution (data_manager.py 방식)
  → Train: 87개 데이터로 적합, Test: 37개 데이터로 적합
  → Valid: 실제 데이터 사용 (분포 샘플링 X)
...
# ============================================================================
# Leadtime Split & Fit (data_manager.py 방식)
# ============================================================================
def split_leadtime_and_fit_gamma(leadtime_data: np.ndarray, train_ratio: float = 0.7):
    """
    data_manager.py 방식: leadtime을 7:3으로 시계열 순서대로 분할 후 gamma 적합
    
    Returns:
        dict: {
            "train_params": {"k": ..., "theta": ...},
            "test_params": {"k": ..., "theta": ...},
            "train_count": int,
            "test_count": int
        }
    ...
    # 시계열 순서대로 분할 (data_manager.py와 동일)
    lt_train = leadtime_data[:n_train]
    lt_test = leadtime_data[n_train:]
    
    print(f"\n[INFO] Leadtime split (data_manager.py 방식 - 시계열 순서):")
    print(f"  - Total: {n_total} pairs")
    print(f"  - Train: {len(lt_train)} pairs ({len(lt_train)/n_total*100:.1f}%)")
    print(f"  - Test: {len(lt_test)} pairs ({len(lt_test)/n_total*100:.1f}%)")
    print(f"  - Train stats: mean={np.mean(lt_train):.2f}, std={np.std(lt_train):.2f}")
    print(f"  - Test stats: mean={np.mean(lt_test):.2f}, std={np.std(lt_test):.2f}")
    
    # data_manager.py의 _fit_lead_params 로직
    train_params = fit_gamma(lt_train, name="train")
    test_params = fit_gamma(lt_test, name="test")
    
    return {
        "train_params": train_params,
        "test_params": test_params,
        "train_count": len(lt_train),
        "test_count": len(lt_test)
    }

def fit_gamma(data: np.ndarray, name: str = "data"):
    """
    Gamma 분포 적합 (data_manager.py 방식: loc=0 고정)
    """
    data = np.array(data)
    data = data[data > 0]
    ...
    if len(data) < 2:
        ...
        return {"k": 1.0, "theta": 7.0}
    
    # Gamma 적합: loc=0 고정
    shape, loc, scale = st.gamma.fit(data, floc=0)
    print(f"[INFO] {name.capitalize()} gamma fit: k={shape:.4f}, theta={scale:.4f}")
    
    return {"k": float(shape), "theta": float(scale)}

def load_item_params_with_leadtime_split(
    ...
    """
    params_master.json 로드 + leadtime train/test 분할 (data_manager.py 방식)
    
    Args:
        use_train: True면 train params (70%), False면 test params (30%)
    """
    with open(params_master_path, 'r') as f:
        master = json.load(f)
    
    # item_key 정규화: "3" → "item3"
    ...
    #  Leadtime: 7:3 분할 + gamma 적합 
    if leadtime_raw_data is not None and len(leadtime_raw_data) > 0:
        print(f"[INFO] Using raw leadtime data for '{item_key}'")
        lt_split = split_leadtime_and_fit_gamma(leadtime_raw_data, train_ratio=train_ratio)
        
        if use_train:
            leadtime_params = lt_split["train_params"]
            count = lt_split["train_count"]
            print(f"[INFO] Using TRAIN leadtime ({count} samples): gamma(k={leadtime_params['k']:.4f}, theta={leadtime_params['theta']:.4f})")
        else:
            leadtime_params = lt_split["test_params"]
            count = lt_split["test_count"]
            print(f"[INFO] Using TEST leadtime ({count} samples): gamma(k={leadtime_params['k']:.4f}, theta={leadtime_params['theta']:.4f})")
        
        leadtime_data = {
            "family": "gamma",
            "k": leadtime_params["k"],
            "theta": leadtime_params["theta"]
        }
    else:
        # JSON에서 로드 (폴백)
        ...
        if lt_family == "gamma":
            leadtime_data = {
                "family": "gamma",
                "k": lt_params["k"],
                "theta": lt_params["theta"]
            }
        elif lt_family == "lognorm":
            leadtime_data = {
                "family": "lognorm",
                "mu": lt_params["mu"],
    ...
    elif demand_family == "poisson":
        lam = demand_params["lam"]
        r_approx = 1000000.0
        p_approx = r_approx / (r_approx + lam)
        demand_converted = {
            "family": "HurdleNB",
            "pi": 0.0,
            "r": r_approx,
...
# ============================================================================
def split_dataframe_item3(df: pd.DataFrame):
    """
    Item3용 342:57:171 (train:valid:test) 순차 분할 - 6:1:3
    """
    ...
    test_weeks = n - train_weeks - valid_weeks  # 나머지를 test에 할당
...
# ============================================================================
# WeeklyInvEnvTest (Test용) -  분포 기반 leadtime 샘플링 
# ============================================================================
class WeeklyInvEnv(gym.Env):
    """CSV 기반 Test 환경 + 분포 기반 leadtime (30%)"""
    def __init__(
        self,
        ...
        self.k = k_past  
        self.cost = cost  
        self.action_levels = np.arange(0, max_order + action_unit, action_unit)
        self.pipeline_horizon = pipeline_horizon
        self.lt_horizon_weeks = pipeline_horizon  # WeeklyInvEnv도 lt_horizon_weeks 사용
        self.reward_scale = reward_scale  # ★ reward scaling factor 저장
        
        # Leadtime 설정
        self.lt_family = leadtime_params.get("family", "gamma")
        if self.lt_family == "gamma":
            self.lt_k = leadtime_params["k"]
            self.lt_theta = leadtime_params["theta"]
            print(f"WeeklyInvEnv: Leadtime gamma(k={self.lt_k:.3f}, theta={self.lt_theta:.3f}) - TEST MODE")
        elif self.lt_family == "lognorm":
            self.lt_mu = leadtime_params["mu"]
            self.lt_sigma = leadtime_params["sigma"]
            print(f"WeeklyInvEnv: Leadtime lognorm(mu={self.lt_mu:.3f}, sigma={self.lt_sigma:.3f}) - TEST MODE")
        else:
    ...
    def sample_leadtime_weeks(self) -> int:
        """leadtime 샘플링 (data_manager.py 방식)"""
        if self.lt_family == "gamma":
            lt = int(np.ceil(self.rng.gamma(shape=self.lt_k, scale=self.lt_theta)))
        elif self.lt_family == "lognorm":
...
# ============================================================================
# GenerativeInvEnv (Training용) -  scale_planned 추가 
# ============================================================================
class GenerativeInvEnv(gym.Env):
    """gamma leadtime + nbinom demand 기반 생성적 환경"""
    def __init__(
        self,
        item_params: dict,
        episode_len: int = 342,
        ...
        scale_backlog=None,
        scale_d: float = None,
        scale_planned: float = None,  # ★★★ 추가 ★★★
        demand_family: str = "HurdleNB",
        reward_scale: float = 10000.0
    ):
        super().__init__()
        ...
        if self.lt_family == "gamma":
            self.lt_k = lt_data["k"]
            self.lt_theta = lt_data["theta"]
            print(f"[GenerativeInvEnv] Leadtime: gamma(k={self.lt_k:.3f}, theta={self.lt_theta:.3f})")
        elif self.lt_family == "lognorm":
        ...
        self.demand_family = demand_family.upper().replace("-", "").replace(" ", "")
        ...
        self.scale_planned = float(scale_planned) if scale_planned is not None else 1.0  # ★★★ 추가 ★★★
    ...
    def sample_demand(self) -> float:
        ...
        if fam == "HURDLENB":
            ...
            if self.rng.random() < pi:
                return 0.0
            for _ in range(100):
                x = self.rng.negative_binomial(r, p)
                if x > 0:
                    return float(x)
            return 1.0
        
        pi = float(cfg.get("pi", 0.0))
        mu = float(cfg.get("mu", 0.0))
        sigma = float(cfg.get("sigma", 1.0))
        if self.rng.random() < pi:
            return 0.0
        return float(np.ceil(self.rng.lognormal(mean=mu, sigma=sigma)))
    ...
    def step(self, action: int):
        ...
        # ★★★ [추가] 여기에 pipeline_qty 계산 로직 추가 ★★★
        pipeline_qty = self.pending_orders.sum()

        info = {
            "t": self.t,
            # Valid/Test와 순서 맞추기 (week_start를 앞으로)
...
class BestModelVisualizationCallback(EvalCallback):
    ...
    def _on_step(self):
        # ★ super()._on_step()이 True를 반환하면 새로운 best model이 저장됨
        continue_training = super()._on_step()
        
        # Evaluation이 실행되었는지 확인
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            self.eval_count += 1
            
            # ★ 새로운 best model이 저장되었는지 확인
            # last_mean_reward는 EvalCallback에서 자동으로 업데이트됨
...
def main():
    ...
    # parser.add_argument("--lead_time_weeks", type=int, default=7)
    parser.add_argument("--lt_horizon_weeks", type=int, default=31)
    ...
    parser.add_argument("--cost_b", type=float, default=0.10)
    parser.add_argument("--cost_c", type=float, default=0.10)
    parser.add_argument("--cost_K", type=float, default=0.00)
    parser.add_argument("--reward_scale", type=float, default=10000.0, help="Reward scaling factor (reward = -cost / reward_scale)")
    parser.add_argument("--action_unit", type=int, default=1)
    parser.add_argument("--max_order", type=int, default=300)
    parser.add_argument("--episodes", type=int, default=37800) #37800
    ...
    # 핵심 수정: Train 리드타임 파라미터만 추출 (70%) 
    item_params_train = load_item_params_with_leadtime_split(
        params_master_path=args.params_master,
        item_key=item_key,
        leadtime_raw_data=leadtime_weeks,
        train_ratio=0.7,
        use_train=True  # Train 70% 리드타임
    )
    
    # Valid와 Test 모두 Train 리드타임 파라미터 사용 
    print(f"\n[INFO] ========== LEADTIME CONFIGURATION ==========")
    if item_params_train['leadtime']['family'] == 'lognorm':
        print(f"[INFO] Train leadtime params: lognormal(mu={item_params_train['leadtime']['mu']:.4f}, sigma={item_params_train['leadtime']['sigma']:.4f})")
    elif item_params_train['leadtime']['family'] == 'gamma':
        print(f"[INFO] Train leadtime params: gamma(k={item_params_train['leadtime']['k']:.4f}, theta={item_params_train['leadtime']['theta']:.4f})")
    else:
        print(f"[INFO] Train leadtime params: {item_params_train['leadtime']}")
    print(f"[INFO] Valid will use TRAIN leadtime params (same as Train)")
    print(f"[INFO] Test will use TRAIN leadtime params (same as Train)")
    print(f"[INFO] ============================================\n")
    
    # Output 디렉토리
    output_dir = os.path.join(args.output_dir, f"item{item_key}_PPO_TrainLT_All")
    ...
    df_train, df_valid, df_test = split_dataframe_item3(df_weekly)
    episode_len = len(df_train)  # 342
    print(f"\n[INFO] Episode length: {episode_len} weeks")
    
    # Scaling (scale_planned 추가)
    ...
    # ============================================
    # Train 환경 (342주, Train 리드타임)
    # ============================================
    train_env_base = GenerativeInvEnv(
        item_params=item_params_train,
    ...
    # ===========================================
    #  Valid 환경 (57주, Train 리드타임 분포 사용) 
    # ============================================
    eval_env = WeeklyInvEnv(  # ← WeeklyInvEnv 대신 WeeklyInvEnvTest 사용!
        weekly_df=df_valid,
        leadtime_params=item_params_train["leadtime"],  # ← Train 리드타임 파라미터 사용!
        k_past=args.k_past,
        cost=cost,
        action_unit=args.action_unit,
        max_order=args.max_order,
    ...
    # # WandB
    run_name = f"per_item{item_key}_PPO_NB_ep{episode_len}_342-57-171split_TrainLT_All"
    wandb.init(
    ...
    lr_schedule = get_linear_fn(2e-4, 2e-5, 1.0)
    ...
    print(f"\n[DEBUG] Checking best model at: {best_model_path}.zip")
    print(f"[DEBUG] File exists? {os.path.exists(best_model_path + '.zip')}")
    
    # Best 모델 로드 (없으면 현재 모델 사용)
    if os.path.exists(best_model_path + ".zip"):
    ...
    # ============================================
    # ★ 전체 데이터로 연속 평가 (Warm-up 방식) - 10번 반복 ★
    # ============================================
    best_model_path = os.path.join(output_dir, "best_model", "best_model")
    
    # Best 모델이 있으면 사용, 없으면 현재 모델 사용
    if os.path.exists(best_model_path + ".zip"):
        print(f"[INFO] Loading BEST model for final evaluation...")
        final_model = PPO.load(best_model_path, device="cpu")
        model_type = "BEST"
    ...
    print(f"\n[INFO] ========== Evaluating {model_type} Model with Warm-up (10 runs) ==========")
    ...
    for run_idx in range(10):
        ...
        # 전체 데이터로 환경 생성 (Train leadtime 사용, 매번 다른 seed)
        full_env = WeeklyInvEnv(
            weekly_df=df_full,
            leadtime_params=item_params_train["leadtime"],
            cost=cost,
            action_unit=args.action_unit,
            max_order=args.max_order,
        ...
        # Test 구간 지표 계산 (실제값)
        # trajectory의 값들은 이미 실제값 (스케일링 안 됨)
    ...
    print(f"  - Train: {len(train_traj_df)} weeks (indices 0-{train_end_idx-1})")
    print(f"  - Valid: {len(valid_traj_df)} weeks (indices {train_end_idx}-{valid_end_idx-1})")
    print(f"  - Test: {len(test_traj_df)} weeks (indices {valid_end_idx}-{len(full_traj_df)-1})")
    ...
    # 파일 저장 확인
    for path in [train_traj_path, valid_traj_path, test_traj_path, full_traj_path]:
        if os.path.exists(path):
            file_size = os.path.getsize(path)
            print(f"[INFO] ✅ Verified: {os.path.basename(path)} ({file_size} bytes)")
    
    # wandb 로깅
    wandb.log({
        "Final/Test_Total_Cost": avg_test_return,
        "Final/Test_Backlog_Avg": avg_test_backlog,
        "Final/Test_Order_Qty_Sum": avg_test_order_qty,
        "Final/Test_Total_Cost_Std": std_test_return,
        "Final/Test_Backlog_Std": std_test_backlog,
        "Final/Test_Order_Qty_Std": std_test_order_qty,
    })
    ...
    if os.path.exists(best_model_path + ".zip"):
        ...
        datasets = {
            ...
            'All': (all_df, train_end, valid_end)  # 경계 정보 포함
        }
