...
Item8 specific:
- Demand: 342:57:171 train/valid/test split (6:1:3) + Negative Binomial distribution
- Leadtime: 87:37 train/test split (7:3 ratio) + Lognormal distribution
...
def split_leadtime_and_fit_lognormal(leadtime_data: np.ndarray, train_ratio: float = 0.7):
    """
    leadtime을 7:3으로 시계열 순서대로 분할 후 lognormal 적합
    Returns:
        dict: {
...
# ============================================================================
def split_dataframe_item8(df: pd.DataFrame):
    """
    Item8용 342:57:171 (train:valid:test) 순차 분할 - 6:1:3
    """
...
class GenerativeInvEnv(gym.Env):
    """★ LOGNORMAL leadtime + NB demand 기반 생성적 환경 ★"""
...
class WandbCallback(BaseCallback):
    ...
    def _visualize_best_episode(self, best_episode):
        ...
        if self.output_dir:
            ...
            print(f"[INFO] Train best trajectory length: {len(traj_df)} weeks")
                ...
                if self.best_model_save_path is not None:
                    ...
                    if os.path.exists(best_model_path + ".zip"):
                        try:
                            ...
                            print(f"[INFO] Valid trajectory length: {len(best_traj)} weeks")
...
def main():
    ...
    parser.add_argument("--lt_horizon_weeks", type=int, default=76)
    ...
    parser.add_argument("--lr", type=float, default=5e-4)  # 3e-4 → 5e-4 (빠른 학습)
    ...
    parser.add_argument("--ent_coef", type=float, default=0.1)  # 0.01 → 0.1 (강력한 exploration!)
    ...
    # ★ Item8 + LOGNORMAL leadtime + NB demand 파라미터 로드 ★
    ...
    print(f"[INFO] Train leadtime params: lognorm(mu={item_params_train['leadtime']['mu']:.4f}, sigma={item_params_train['leadtime']['sigma']:.4f})")
    ...
    # Output 디렉토리
    output_dir = os.path.join(args.output_dir, f"item{item_key}_PPO_Lognormal_NB_TrainLT_All")
    ...
    df_train, df_valid, df_test = split_dataframe_item8(df_weekly)
    ...
    # WandB
    run_name = f"(yena)per_item{item_key}_PPO_Lognormal_NB_ep{episode_len}_6-1-3split_TrainLT_All"
    wandb.init(
    ...
    print(f"\n[INFO] Leadtime configuration:")
    print(f"  - Train: lognorm(mu={item_params_train['leadtime']['mu']:.4f}, sigma={item_params_train['leadtime']['sigma']:.4f})")
